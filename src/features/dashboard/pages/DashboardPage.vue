...
function formatDate(d: string) {
  try {
    return format(parseISO(d), "dd 'de' MMM", { locale: ptBR })
  } catch {
    return d
  }
}
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <template v-else>
          <BaseCard padding="md" class="group animate-rise relative overflow-hidden">
            <div
              class="absolute -top-8 -right-8 w-24 h-24 rounded-full bg-[#7C5CFC]/5 group-hover:bg-[#7C5CFC]/10 transition-colors"
              aria-hidden="true"
            />
            <div class="flex items-start justify-between mb-3 relative">
              <div
                class="w-11 h-11 rounded-2xl bg-gradient-to-br from-[#8B5CF6] to-[#6366F1] flex items-center justify-center shadow-[0_8px_18px_-6px_rgba(124,92,252,0.6)] group-hover:scale-105 transition-transform"
              >
                <Users :size="20" class="text-white" />
              </div>
              <BaseBadge variant="primary" size="sm">+2 mês</BaseBadge>
            </div>
            <p class="text-3xl font-bold text-[#1E293B] tracking-tight relative">
          ...
          <BaseCard
            padding="md"
            class="group animate-rise relative overflow-hidden"
            style="animation-delay: 60ms"
          >
            <div
              class="absolute -top-8 -right-8 w-24 h-24 rounded-full bg-[#3B82F6]/5 group-hover:bg-[#3B82F6]/10 transition-colors"
              aria-hidden="true"
            />
            <div class="flex items-start justify-between mb-3 relative">
              <div
                class="w-11 h-11 rounded-2xl bg-gradient-to-br from-[#3B82F6] to-[#2563EB] flex items-center justify-center shadow-[0_8px_18px_-6px_rgba(59,130,246,0.6)] group-hover:scale-105 transition-transform"
              >
          ...
          >
            <div
              class="absolute -top-8 -right-8 w-24 h-24 rounded-full bg-[#22C55E]/5 group-hover:bg-[#22C55E]/10 transition-colors"
              aria-hidden="true"
            />
            <div class="flex items-start justify-between mb-3 relative">
              <div
                class="w-11 h-11 rounded-2xl bg-gradient-to-br from-[#22C55E] to-[#16A34A] flex items-center justify-center shadow-[0_8px_18px_-6px_rgba(34,197,94,0.6)] group-hover:scale-105 transition-transform"
              >
          ...
          >
            <div
              class="absolute -top-8 -right-8 w-24 h-24 rounded-full bg-[#F59E0B]/5 group-hover:bg-[#F59E0B]/10 transition-colors"
              aria-hidden="true"
            />
            <div class="flex items-start justify-between mb-3 relative">
              <div
                class="w-11 h-11 rounded-2xl bg-gradient-to-br from-[#F59E0B] to-[#D97706] flex items-center justify-center shadow-[0_8px_18px_-6px_rgba(245,158,11,0.6)] group-hover:scale-105 transition-transform"
              >
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <BaseCard padding="md">
          ...
          <apexchart
            ...
            :series="[
              { name: 'Receitas', data: chartReceitas },
              { name: 'Despesas', data: chartDespesas },
            ]"
          />
        </BaseCard>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <BaseCard padding="none">
          <div class="flex items-center justify-between px-6 py-4 border-b border-[#F1F5F9]">
            ...
            <RouterLink
              to="/agenda"
              class="text-sm text-[#7C5CFC] hover:underline flex items-center gap-1"
            >
          ...
          <ul v-else class="divide-y divide-[#F8FAFC]">
            ...
            >
              <div
                class="w-9 h-9 rounded-xl bg-[#EDE9FE] flex items-center justify-center flex-shrink-0"
              >
