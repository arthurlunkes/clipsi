...
const finalizados = computed(() =>
  fila.value.filter((f) => f.status === 'finalizado' || f.status === 'ausente'),
)
...
async function save() {
  if (!form.value.pacienteId) {
    errors.value.pacienteId = 'Selecione um paciente'
    return
  }
  saving.value = true
  try {
...
const prioridadeVariant = (p: number): any =>
  ({ 1: 'danger', 2: 'warning', 3: 'neutral' })[p] ?? 'neutral'
  ...
  <div class="p-4 lg:p-6 max-w-5xl mx-auto space-y-5">
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex gap-2">
        ...
        <BaseButton @click="openNew"> <Plus :size="16" /> Adicionar </BaseButton>
      </div>
    </div>
    ...
    <template v-else>
      ...
      <div v-if="aguardando.length > 0">
        <h2
          class="text-sm font-semibold text-[#64748B] uppercase tracking-wide mb-2 flex items-center gap-2"
        >
        ...
        <div class="space-y-2">
          ...
          >
            <div class="flex items-center gap-3 p-4">
              <div
                class="w-10 h-10 rounded-full bg-[#FEF3C7] flex items-center justify-center flex-shrink-0"
              >
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-2 mt-0.5">
                  ...
                  <p class="text-xs text-[#F59E0B] font-medium">
                    {{ tempoEspera(f.horaChegada) }} esperando
                  </p>
                </div>
              </div>
              <div class="flex items-center gap-2">
                <BaseBadge :variant="prioridadeVariant(f.prioridade)" size="sm">{{
                  prioridadeLabel(f.prioridade)
                }}</BaseBadge>
                <div class="flex gap-1">
                  <button
                    @click="updateStatus(f.id!, 'em_atendimento')"
                    class="p-2 text-[#7C5CFC] hover:bg-[#EDE9FE] rounded-lg transition-colors"
                    aria-label="Iniciar atendimento"
                  >
                    <Play :size="15" />
                  </button>
                  <button
                    @click="updateStatus(f.id!, 'ausente')"
                    class="p-2 text-[#F59E0B] hover:bg-[#FEF3C7] rounded-lg transition-colors"
      ...
      <div v-if="emAtendimento.length > 0">
        ...
        <div class="space-y-2">
          <BaseCard
            v-for="f in emAtendimento"
            :key="f.id"
            padding="none"
            class="border-l-4 border-l-[#7C5CFC]"
          >
            <div class="flex items-center gap-3 p-4">
              <div
                class="w-10 h-10 rounded-full bg-[#EDE9FE] flex items-center justify-center flex-shrink-0"
              >
              ...
              <button
                @click="updateStatus(f.id!, 'finalizado')"
                class="p-2 text-[#22C55E] hover:bg-[#DCFCE7] rounded-lg transition-colors"
        ...
        <div class="space-y-2">
          <BaseCard v-for="f in finalizados" :key="f.id" padding="none" class="opacity-70">
            <div class="flex items-center gap-3 p-3">
              <div
                class="w-9 h-9 rounded-full bg-[#F1F5F9] flex items-center justify-center flex-shrink-0"
              >
