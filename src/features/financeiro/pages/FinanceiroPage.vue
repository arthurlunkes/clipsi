...
const categoriaOptions = computed(() => form.value.tipo === 'receita' ? categoriaReceitaOptions : categoriaDespesaOptions)
...
const totalReceitas = computed(() => filtered.value.filter(i => i.tipo === 'receita' && i.status === 'pago').reduce((s, i) => s + i.valor, 0))
const totalDespesas = computed(() => filtered.value.filter(i => i.tipo === 'despesa' && i.status === 'pago').reduce((s, i) => s + i.valor, 0))
const lucro = computed(() => totalReceitas.value - totalDespesas.value)
const contasReceber = computed(() => filtered.value.filter(i => i.tipo === 'receita' && i.status === 'pendente').reduce((s, i) => s + i.valor, 0))
...
function openNew() {
  form.value = { tipo: 'receita', descricao: '', valor: 0, data: format(now, 'yyyy-MM-dd'), status: 'pago', categoria: '', observacoes: '' }
...
function openEdit(l: Lancamento) {
  form.value = { tipo: l.tipo, descricao: l.descricao, valor: l.valor, data: l.data, status: l.status, categoria: l.categoria, observacoes: l.observacoes ?? '' }
...
function validate() {
  ...
  if (!form.value.valor || form.value.valor <= 0) errors.value.valor = 'Valor deve ser maior que zero'
  ...
  <div class="p-4 lg:p-6 max-w-6xl mx-auto space-y-5">
    ...
    <div class="flex flex-wrap items-center gap-3">
      ...
      <div class="flex gap-2 ml-auto">
        <BaseSelect id="filterTipo" v-model="filterTipo" :options="[{ label: 'Todos', value: '' }, ...tipoOptions]" />
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <BaseCard padding="md">
        ...
        <p class="text-xl font-bold" :class="lucro >= 0 ? 'text-[#16A34A]' : 'text-[#DC2626]'">{{ fmtMoney(lucro) }}</p>
      </BaseCard>
        ...
        <table class="w-full" aria-label="Lançamentos financeiros">
          <thead>
            <tr class="border-b border-[#F1F5F9]">
              <th class="text-left px-6 py-3 text-xs font-semibold text-[#64748B] uppercase tracking-wide">Descrição</th>
              <th class="text-left px-6 py-3 text-xs font-semibold text-[#64748B] uppercase tracking-wide hidden sm:table-cell">Categoria</th>
              <th class="text-left px-6 py-3 text-xs font-semibold text-[#64748B] uppercase tracking-wide hidden md:table-cell">Data</th>
              <th class="text-left px-6 py-3 text-xs font-semibold text-[#64748B] uppercase tracking-wide">Valor</th>
              <th class="text-left px-6 py-3 text-xs font-semibold text-[#64748B] uppercase tracking-wide hidden sm:table-cell">Status</th>
            ...
            <tr v-for="l in filtered" :key="l.id" class="hover:bg-[#F8FAFC] transition-colors">
              <td class="px-6 py-3">
                <div class="flex items-center gap-2">
                  <div class="w-1 h-8 rounded-full flex-shrink-0" :class="l.tipo === 'receita' ? 'bg-[#22C55E]' : 'bg-[#EF4444]'" />
                  <div>
              ...
              <td class="px-6 py-3 text-sm text-[#64748B] hidden sm:table-cell">{{ l.categoria }}</td>
              <td class="px-6 py-3 text-sm text-[#64748B] hidden md:table-cell">{{ fmtDate(l.data) }}</td>
              <td class="px-6 py-3 text-sm font-semibold" :class="l.tipo === 'receita' ? 'text-[#16A34A]' : 'text-[#DC2626]'">
              ...
              <td class="px-6 py-3">
                <div class="flex items-center justify-end gap-1">
                  <button @click="openEdit(l)" class="p-2 text-[#64748B] hover:text-[#1E293B] hover:bg-[#F1F5F9] rounded-lg transition-colors" aria-label="Editar">
                    <Pencil :size="15" />
                  </button>
                  <button @click="remove(l.id!)" class="p-2 text-[#64748B] hover:text-[#EF4444] hover:bg-[#FEE2E2] rounded-lg transition-colors" aria-label="Excluir">
                    <Trash2 :size="15" />
                  </button>
                </div>
              </td>
    ...
    <!-- Modal -->
    <BaseModal :open="showModal" :title="editingId ? 'Editar lançamento' : 'Novo lançamento'" size="md" @close="showModal = false">
      <form @submit.prevent="save" class="space-y-4" novalidate>
        ...
        <BaseInput id="descricaoL" v-model="form.descricao" label="Descrição" placeholder="Ex: Sessão de psicoterapia" :error="errors.descricao" required />
        <div class="grid grid-cols-2 gap-4">
          <BaseInput id="valorL" v-model="form.valor" type="number" label="Valor (R$)" :error="errors.valor" required />
          <BaseInput id="dataL" v-model="form.data" type="date" label="Data" :error="errors.data" required />
        </div>
        <div class="grid grid-cols-2 gap-4">
          <BaseSelect id="categoriaL" v-model="form.categoria" label="Categoria" :options="categoriaOptions" placeholder="Selecione" :error="errors.categoria" required />
          <BaseSelect id="statusL" v-model="form.status" label="Status" :options="statusOptions" required />
        </div>
